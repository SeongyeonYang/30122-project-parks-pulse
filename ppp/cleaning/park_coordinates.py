...
def get_coordinates():
    """Using nps API to get coordinates

    Returns:
        dataframe: a dataframe that contains park's coordinates
    """
    ...
    data_all = (
        response.json()
    )  # Interpret bytes as JSON format and convert to a Python dict
    ...
    for park in data_all["data"]:
        ...
        if "National Park" in fullName:
            parks_data.append(
                {"Name": fullName, "latitude": latitude, "longitude": longitude}
            )
    parks_df = pd.DataFrame(parks_data)
    filename = pathlib.Path(__file__).parent / \
        "cleaning/data/parks_coordinates.csv"
