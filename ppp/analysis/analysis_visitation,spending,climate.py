...
def visualize_yearly_trends(data):
    ...
    Note: Visitation trends generally show an upward trajectory, 
        ...
        increased outdoor activity popularity, park accessibility improvements, 
        or heightened awareness. 
        However, a notable decrease was observed during 2022-2023, 
        likely attributed to the COVID-19 pandemic. 
        spending trends have also has a similar characteristics
    """
    ...
    ax1.plot(data['Year'].unique(), data.groupby('Year')['Visitation'].sum(), 
             color=color)
    ...
    ax2.plot(data['Year'].unique(), data.groupby('Year')['Spending'].sum(), 
             color=color)
...
# 3. Visualize yearly changes in the number of visitors across all parks 
# on a single graph (line graph):
def visualize_park_visitation_trends(data):
    ...
    This function plots a line graph for each park showing the change in 
    ...
    for park in unique_parks:
        ...
        plt.plot(park_data['Year'], park_data['Visitation'], marker='o', 
                 linestyle='-', label=park)
...
# 4. Visualize yearly changes in the number of visitors across 
# all parks on a single graph (area chart):
def visualize_park_visitation_trends_area(data):
    ...
    This function plots an area chart for each park showing 
    the change in visitation over the years.
    The area under each park's line is filled with color, 
    and parks are stacked upon each other.
    """
    # Getting unique park names
    unique_parks = data['Park Name'].unique()
    plt.figure(figsize=(14, 8))
    ...
    for park in unique_parks:
        ...
        plt.fill_between(park_data['Year'], park_data['Visitation'], 
                         label=park, alpha=0.5)
...
def compute_and_visualize_correlation(data):
    ...
    data_relevant = data[['Spending', 'Visitation', 'temp_avg', 'precip_sum', 
    ...
    sns.heatmap(correlation_matrix_cleaned, annot=True, fmt=".2f", cmap='coolwarm', 
                xticklabels=correlation_matrix_cleaned.columns, 
                yticklabels=correlation_matrix_cleaned.columns)
...
def analyze_growth_rate(data):
    """
    Analyzes the growth rate of visitation for each park and identifies 
    the park with the highest and lowest average growth rate.
    ...
    Note: This analysis helps identify parks that are rapidly 
        gaining or losing popularity. 
        The park with the fastest growth rate is Kobuk Valley, 
        with an infinite average growth rate, 
        indicating significant changes in the number of visitors. 
        This could occur when there is a substantial 
        increase from years with very few visitors. 
        The park with the greatest decline is Olympic, 
        ith an average growth rate of approximately -1.32%.
    """
    # Calculate the change (%) in visitation and the average growth rate 
    # for each park by year
    visitation_growth = data.pivot_table(index='Year', columns='Park Name', 
...
def compare_covid_period(data):
    ...
    and during the COVID-19 period (2020-2023).
    
    Args:
    - data: pandas DataFrame containing 'Year', 'Spending', 'Visitation', 
    and climate variables.
    
    Prints summary statistics for the COVID-19 period (2020-2023) 
    ...
    - Visitation and Spending: In 2020, the average number of visitors 
                            was about 1,078,281, 
                            with an average expenditure of $7,734. In 2021, 
                            the average number of visitors increased 
                            to approximately 1,464,315, 
                            with spending at an average of $7,907. 
                            In 2022, the visitation averaged around 1,407,336, 
                            with an average spending of $8,050. Data for 2023 
                            shows missing visitor numbers, with an average 
                            expenditure of $7,889.
    - Climate Features: Average temperature, precipitation, visibility, and UV index 
                        showed minor changes during this period, 
                        with no significant overall deviations. 
                        In 2022 and 2023, severerisk data began to appear.
    - Comparison with the Preceding Period (before 2019): 
        Average expenditure and visitor numbers show no significant difference 
        compared to the period before 2020. 
    ...
    summary_pre_2020 = data_pre_2020.groupby('Year').mean().mean()  
