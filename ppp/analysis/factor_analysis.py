...
park_code_df = pd.read_csv(
    f"{csv_filepath}/nps-parkcode.csv",
    usecols=["SHORT", "REGION"],
    encoding="ISO-8859-1",
)
park_code_df.rename(columns={"SHORT": "Park Name"}, inplace=True)
park_code_df["Park Name"] = park_code_df["Park Name"].replace(
    "National Park of American Samoa", "American Samoa"
)
park_code_df["Park Name"] = park_code_df["Park Name"].replace(
    "Hawai'i Volcanoes", "Hawaii Volcanoes"
)
park_code_df["Park Name"] = park_code_df["Park Name"].replace(
...
avg_temp_2011 = (
    time_series_df[time_series_df["Year"] == 2011]
    .groupby("Park Name")["temp_avg"]
    .mean()
    .reset_index()
)
avg_temp_2011.columns = ["Park Name", "First_Year_Avg_Temp"]
time_series_df = time_series_df.merge(avg_temp_2011, on="Park Name")
time_series_df["temp_avg"] = (
    time_series_df["temp_avg"] / time_series_df["First_Year_Avg_Temp"]
)
...
time_series_df[["acres", "count"]] = \
    time_series_df[["acres", "count"]].fillna(value=0)
...
def data_prep(df):
    """
    Prepare data for running factor analysis

    Args:
        df (pd.dataframe): our data

    Returns:
        pd.dataframe: the data that is ready for factor analysis
    """
    ...
    df_fix["water_facility"] = df_fix["Waste Water Systems"] / (
        df_fix["Buildings"] + df_fix["Housing Units"] + 
        df_fix["Campgrounds"] + 1
    )
    df_fix = df_fix.drop(
        columns=[
            "Trail Miles",
    ...
    negative_impact_vars = [
        "Abandoned_wells_within_30_miles",
        "Active_wells_in_parks",
        "At_risk",
        "temp_avg",
        "visibility",
        "uvindex",
...
def adequacy_test(df_scaled, year):
    """Adequacy test, and some graph for the test

    Args:
        df_scaled (pd.dataframe): input data(after standardization)
        year (int): the year we are analyzing

    Returns:
        df_eigen: the factors dataframe we get
    """
    # Correlation
    ...
    df_communalities = pd.DataFrame(
        data={"Column": df_scaled.columns, "Communality": fa.get_communalities()}
    )
    ...
    communality_bar = (
        ...
        + geom_bar(aes(x="Column", y="Communality"), width=0.75, stat="identity")
    ...
    # Save the graph
    communality_bar.save(
        filename=f"{vis_filepath}/{year}_communality_bar.png", 
                    dpi=1000, verbose=False
    )
    ...
    df_eigen = pd.DataFrame(
        {"Factor": range(1, len(eigenValue) + 1), "Eigen value": eigenValue}
    )
    ...
    scree_eigenvalue = (
        ...
        + geom_line(aes(x="Factor", y="Eigen value"))
        + geom_point(aes(x="Factor", y="Eigen value"), size=2)
    ...
    # Save the graph
    scree_eigenvalue.save(
        filename=f"{vis_filepath}/{year}_scree_eigenvalue.png", 
                    dpi=1000, verbose=False
    )
    return df_eigen
...
def factor_analysis(df, df_scaled, df_eigen, year):
    ...
    Args:
        ...
        df_scaled (pd.dataframe): the standardized data
        df_eigen (pd.dataframe): factors data
        year (int): the year we are analyzing
    """
    # Number of factors
    ...
    df_ratio_var = pd.DataFrame(
        data=ratioVariance.reshape((1, n_factor)),
    ...
    df_factors = pd.DataFrame(
        data=fa.fit_transform(df_scaled),
    ...
    df_factors_scaled = pd.DataFrame(
        data=scaler.fit_transform(df_factors),
        index=pd.MultiIndex.from_frame(df[["Park Name", "REGION"]]),
        columns=facs,
    )
    ...
    for i in range(n_factor):
        ...
        value = df_factors_scaled.iloc[:, i].values * \
            df_ratio_var.iloc[:, i].values
        dict_index.update({key: value})

    # Create a dataframe
    df_index = pd.DataFrame(
        dict_index, index=pd.MultiIndex.from_frame(df[["Park Name", "REGION"]])
    )
    ...
    stat = [
        "Above" if i > df_index["Composite Index"].mean() else "Below"
        for i in df_index["Composite Index"]
    ]
    ...
    composite_index_distribution = (
        ...
        + geom_density(aes(x="Composite Index"), color="white", fill="#c22d6d")
    ...
    # Save the graph
    composite_index_distribution.save(
        filename=f"{vis_filepath}/{year}_composite_index_distribution.png",
        dpi=1000,
        verbose=False,
    )


# Run 2015 and 2019 factor analysis
# 2015
year = 2015
...
index_2015 = pd.read_csv(
    f"{csv_filepath}/2015_factor_table.csv",
    usecols=["Park Name", "REGION", "Composite Index"],
).drop_duplicates()

average_composite_index = (
    index_2015.groupby("REGION")["Composite Index"].mean().reset_index()
)
plt.figure()
sns.barplot(
    x="REGION", y="Composite Index", data=average_composite_index, palette="viridis"
)
plt.xlabel("Region")
